...
module.exports = {
  ...
  plugins: [
    new ModuleFederationPlugin({
      ...
      exposes: {
        './UserReport': './src/pages/UserReport',
        './ReportDashboard': './src/components/ReportDashboard',
        './ReportList': './src/components/ReportList',
        './ReportAnalytics': './src/components/ReportAnalytics',
      },
      shared: {
        react: {
          singleton: true,
  ...
  output: {
    path: path.resolve(__dirname, '../../dist/apps/user-report'),
    filename: isDevelopment ? '[name].js' : '[name].[contenthash].js',
    chunkFilename: isDevelopment ? '[name].chunk.js' : '[name].[contenthash].chunk.js',
    clean: true,
    publicPath: isDevelopment ? 'http://localhost:4201/' : '/user-report/',
  },
};